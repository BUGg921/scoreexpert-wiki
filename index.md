...
`raw/articles/` 当前保存 8 份快照，覆盖 7 个成熟场景：同构、单慢卡、三种双慢卡拓扑、四慢卡和五慢卡 2/1/1/1；S7 旧快照只保留历史审计。成熟场景都属于**延迟优先型**；稳定优先型目前没有场景经验。

新场景同时命中延迟优先总览规则和对应 raw 场景边界后，直接推理部署策略，无需重新运行真实 Evaluation；只有未命中、越界或冲突时才进入补库流程。

当前正式知识页为 ScoreExpert 入口、部署经验总库和延迟优先经验总览；7 个具体场景已审核准入，详细证据保存在 `raw/articles/`。
...
- [同节点双慢卡场景](raw/articles/two-slow-gpu-same-node-evolve-analysis-2026-07-30.md) — 使用 `PP=4,TP=8,DP=1,MBN=8` 将影响限制在一个节点内 group/stage。
...
- [五张慢卡 2/1/1/1 Evolve 场景](raw/articles/five-slow-gpu-2-1-1-1-evolve-analysis-reviewed-2026-07-30.md) — 重新审核准入 `PP=16,TP=1,DP=2,MBN=64`；旧快照只保留审计。
