---
source_url:
source_path: /Users/cookie/Documents/wiki/DAGBuilder_Evolve/outputs/s7-five-slow-2-1-1-1_20260730_144533/scenario_analysis.md
ingested: 2026-07-30
sha256: 3f6aa1043325afcd869d8d7029a1dcfbeb06f27289cceffbfcb507edc6ab86cf
original_sha256: 3896fd84269edf7701961cf5968700af02f25cc9c072024f6615556fc95e6c48
---
...
基于 Evolve 正式搜索、DAG 数值仿真、当前已仿真最优解和实际提名该候选的打分策略代码，总结本场景部署策略，并将并行策略、参数选择原因和结论边界分开表述。
...
1. 当异常设备跨多个节点或亲和组、无法通过一个局部执行单元完成隔离，且`保留满卡与数据并行的收益 > replica等待和DP通信成本`时，优先采用满卡方案。参数按以下规则求解：先取能够限制异常同步扩散的最小已验证`TP=1`；再保留本轮仿真有收益的`DP=2`；在满卡约束下由`PP=active_gpu/(TP×DP)=32/(1×2)=16`反推PP；最后取显存、气泡和调度边界内已验证的`MBN=64`。当前实例得到`PP=16, TP=1, DP=2, MBN=64`，即`16×1×2=32`；当前证据只支持下述固定Rank映射；若要按各DP replica的预测执行时间重新均衡异常卡，必须把新映射作为独立场景复仿。当前代表解为`1f1b/allreduce`；MBN处于搜索上界，不能外推为越大越好；具体Rank映射为：Rank 6→节点0/亲和组0/stage3/DP副本0/TP lane0；Rank 7→节点0/亲和组0/stage3/DP副本1/TP lane0；Rank 15→节点1/亲和组0/stage7/DP副本1/TP lane0；Rank 23→节点2/亲和组1/stage11/DP副本1/TP lane0；Rank 31→节点3/亲和组1/stage15/DP副本1/TP lane0。
