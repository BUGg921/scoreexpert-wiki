...
- 若以后因独立治理需要重新建立具体经验页，其正文使用 `1. 场景描述` 和 `2. 具体的并行策略`；当前七个成熟场景不创建重复经验页，直接由目标总览与 raw 来源共同承载。
...
三类知识的“并行策略”必须写成“可复用分布条件 + 收益关系 + 参数求解规则 + 拓扑映射”：不能使用“两慢卡非对称均衡”“四慢卡对称副本”等具体案例名称作为策略标题，也不能用固定慢卡数量代替类别规则；具体数量和案例名称只写在“场景案例”。条件明确后再给出 `PP/TP/DP/MBN` 的取值规则、`TP:DP` 比例、`PP×TP×DP=active_gpu` 和 group/replica/stage 映射；固定参数元组只作为场景案例。“原因”逐项解释为什么这些条件会导向该参数组合。“场景案例”使用普通 Markdown 链接指向对应 raw 来源。

局部异构的“场景定义”必须同时说明：异常性能差异是否稳定可识别，异常 rank/速度/位置是否已知，异常是否集中或可映射到一个局部执行单元，局部范围之外是否仍保持无固定快慢结构，模型是否支持 rank/group/stage/layer/计算量重映射，以及无法收敛时如何改判为分布式异构。慢卡数量不能单独作为局部异构的判定标准。
...
优化目标总览保存该目标下的三类场景规则。三类分支在“并行策略”中写入成熟的参数求解规则，在“场景案例”中保留具体参数并链接唯一 raw 来源；更细的卡数、慢卡位置、rank 映射、Score 代码和结论边界在 raw 中维护。不得设置“当前经验范围”。
