...
### 直接推理契约
- **硬匹配字段**：<优化目标、类别、资源拓扑、异构分布、模型约束、映射能力>
- **允许变换**：<可以按哪些比例、阈值或公式调整实例参数>
- **直接输出**：<主策略、PP/TP/DP/MBN、映射、置信度和回退>
- **停止条件**：<缺字段、越界、冲突或非 active 时转入何种补库任务>
...
### 分布式异构的影响
### 对策：均衡与对称
### 场景案例（使用案例一、案例二等具体名称，分别链接并概括对应经验卡）
```
