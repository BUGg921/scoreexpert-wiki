...
### 场景定义
### 并行策略（写触发条件、具体 PP/TP/DP/MBN、TP:DP 比例和映射摘要）
### 原因（使用与并行策略相同的编号，并在同一原因条目内解释不对称均衡与对称映射）
### 场景案例（按非对称、对称等具体案例概括并链接 raw 场景来源）
```

三类分支的“并行策略”直接写成熟的参数求解规则，包括 `PP/TP/DP/MBN` 取值、`TP:DP`、`PP×TP×DP=active_gpu` 和完整拓扑约束；“原因”只解释参数机制；“场景案例”概括具体条件和数值结果并链接 raw 来源。详细卡数、慢卡位置、rank 映射、Score 代码和结论边界只在 raw 来源维护。优化目标总览不设置“当前经验范围”，也不插入重复分类页或具体经验卡；稳定优先分支缺少经验时明确写“知识缺口”，不编造参数。
