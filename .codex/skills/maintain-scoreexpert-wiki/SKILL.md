...
- 当前使用“总经验库 → 优化目标总体经验 → 具体场景经验卡”的三层结构，不再维护重复的分类总览。目标总览只写跨实例总体规则；具体卡数、拓扑和参数元组放入“场景案例”并链接唯一经验卡，准入状态、审核记录和证据完整度只在具体经验卡维护。案例直接使用“标准 32 卡同构基线”等业务名称，不添加“单场景经验”等重复标签；不得在总体规则中设置“当前经验范围”或混入“当前实例”。
