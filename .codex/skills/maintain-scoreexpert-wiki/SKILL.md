...
- 具体场景经验卡正文固定只有 `1. 场景描述` 和 `2. 具体的并行策略` 两个一级章节。`status`、`optimization_priority`、`admitted_by`、`admitted_at`、`sources` 和 `confidence` 属于治理 frontmatter，不在正文重复；第一章容纳拓扑、异构、模型/映射约束、硬匹配条件和不适用条件，第二章只使用“部署策略、部署经验、失效条件与回退”。执行步骤并入部署策略，适用范围归入第一章。
- active 经验的直接推理契约必须完整嵌入这两个章节：硬匹配字段放在“场景描述”，允许变换、部署策略和停止/补库条件放在“具体的并行策略”。只有参数结果或证据审计不算经验总结。
...
- 当前使用“总经验库 → 优化目标总体经验 → 具体场景经验卡”的三层结构，不再维护重复的分类总览。目标总览只写跨实例总体规则；具体卡数、拓扑和参数元组放入“场景案例”并链接唯一经验卡，准入状态、审核记录和证据完整度只在具体经验卡的 frontmatter、来源层或审核日志维护。案例直接使用“标准 32 卡同构基线”等业务名称，不添加“单场景经验”等重复标签；不得在总体规则中设置“当前经验范围”或混入“当前实例”。
...
- 隔离、均衡与对称等总体对策必须融入 `TP`、`TP/PP`、`DP`、`PP/MBN` 的具体规则，不单列“总体对策”“隔离对策”或“均衡与对称对策”条目。
- 优化目标总览中的局部异构和分布式异构知识固定使用“场景定义 → 异构影响 → 并行策略 → 场景案例”。隔离、均衡与对称等对策属于并行策略，不另设并列章节；具体实例独立放入最后的场景案例。
...
- 经验成熟度与来源附件完整度分开判断。知识库所有者明确确认经验成熟时，可作为人工审核准入依据；补齐直接推理契约和边界，在 frontmatter 记录 `status: active`、`admitted_by`、`admitted_at` 并通过 lint，同时在来源层或审核日志如实保留缺失的原始指标说明。
