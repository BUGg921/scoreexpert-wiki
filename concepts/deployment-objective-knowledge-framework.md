...
- 记录总卡数、节点数、每节点卡数、亲和组和实际启用卡数。
- 满卡与少卡必须作为可比较选择，不能默认“卡越多延迟越低”。

#### (3) 并行策略

- 记录 `PP`、`TP`、`DP`、`MBN` 及 rank/group/stage 映射。
- 解释每个维度对通信、流水线 bubble、同步等待和单步延迟的影响。
...
- 记录总卡数、启用卡数、节点边界和冗余/降级容量。
- 比较满卡与保留余量时的 OOM、失败率和性能波动。

#### (3) 并行策略

- 记录 `PP`、`TP`、`DP`、`MBN`。
- 重点检查显存余量、通信超时、重复运行方差、P99 和失败恢复行为。
