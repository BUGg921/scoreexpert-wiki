...
1. **满卡方案**
   - **`PP` 取最小值**：减少流水线 stage 和 pipeline bubble；模型显存允许时优先不切 PP。
...
2. 当多张异常卡位于同一节点并能够收敛到同一个节点内 TP group/stage，且保留较大节点内 TP 的并行收益高于缩小 TP 后增加 PP 深度的收益时，取避免 replica skew 的 `DP=1`；`TP` 取不跨节点且能够容纳该局部异常集合的最大已验证值；由 `PP=active_gpu/(TP×DP)` 反推整数 PP；`MBN` 取显存、气泡和调度边界内的最大可行值。异常卡必须仍能落入同一个局部 group/stage，映射变化后重新仿真。

### (3) 原因

1. **单个局部 group/stage 隔离**
   - **`DP` 取最小值**：避免形成纯快 replica 等待含慢卡 replica 的跨副本同步；优先 `DP=1` 时，`TP:DP=2:1` 对应 `TP=2`。
   ...
   - **`MBN` 取最大可行值**：深 PP 需要更多微批降低 pipeline bubble，但上限必须由显存、端到端延迟和调度开销约束；32 卡案例中的 `MBN=64` 是搜索上界，不是所有资源规模的固定值。

2. **同节点多慢卡局部收敛**
   - **`DP=1`**：避免跨副本快慢等待。
   - **节点内 `TP`**：将同步影响限制在该节点，同时保留更多张量并行收益。
   - **`PP`**：在满卡约束下反推，避免无条件追求更深流水线。
...
- **同节点双慢卡局部收敛**：[场景来源](../raw/articles/two-slow-gpu-same-node-evolve-analysis-2026-07-30.md)在两张约 0.5× 慢卡位于同一节点并落入同一局部 group/stage 时，使用 `PP=4, TP=8, DP=1, MBN=8`。
...
2. 当异常设备跨节点或亲和组、无法收敛到一个局部执行单元，且消除 TP 集合通信与保留两路 DP 的收益大于深 PP 气泡、replica skew 和 DP 通信成本时，取 `TP=1,DP=2`；由 `PP=active_gpu/(TP×DP)` 反推整数 PP；`MBN` 取显存、气泡和调度边界内能够填充深流水线的最大可行值。每个 TP group 只包含单卡；只在审核后 raw 的固定场景与映射边界内直接召回，改变 Rank 映射时重新仿真。

### (3) 原因

1. **最小 PP 与 `TP:DP=2:1` 分支**
   - **共同参数**：慢卡跨多个区域时，深 PP 容易形成多个慢 stage，因此 `PP` 取最小值以避免多个 stage 瓶颈和 pipeline bubble；`TP:DP=2:1` 保留成熟经验中的并行比例，再根据可用卡数求解完整 TP group 和 DP replica；最小 PP 不需要更多微批填充深流水线，因此 `MBN` 也取最小可行值。
   - **replica 不对称**：各 replica 的异常结构或预计耗时不一致时，固定位置映射会产生快慢 replica 等待；按预测执行时间调整异常卡位置可以缩小 replica step-time 差异。
   - **replica 可对称**：异常卡能够按数量、速度和位置对称分配时，使各 replica 的异常结构一致，可以让慢卡影响在副本间近似同步，减少由结构差异造成的等待。高频 TP 通信仍留在带宽最高的局部拓扑内。

2. **`TP=1,DP=2` 深 PP 分支**
   - **`TP=1`**：消除慢卡通过 TP 集合通信扩散的等待。
   - **`DP=2`**：在使用全部 32 张卡时保留数据并行收益。
   - **`PP/MBN`**：由 `TP` 和 `DP` 反推的深 PP 需要较大 MBN 降低气泡。
...
- **同亲和组跨节点双慢卡**：[场景来源](../raw/articles/two-slow-gpu-same-affinity-evolve-analysis-2026-07-30.md)在两张约 0.5× 慢卡位于同一亲和组的两个节点时，使用 `PP=16, TP=1, DP=2, MBN=64`。
- **四慢卡对称副本**：[场景来源](../raw/articles/four-slow-gpu-deployment-analysis-2026-07-22.md)在四张速度接近的慢卡一节点一张时，使用相同参数构造慢卡结构对称的 DP replica。
- **五慢卡 2/1/1/1 深 PP**：[审核后场景来源](../raw/articles/five-slow-gpu-2-1-1-1-evolve-analysis-reviewed-2026-07-30.md)在五张约 0.5× 慢卡按节点 2/1/1/1 分布时，使用 `PP=16, TP=1, DP=2, MBN=64`；旧快照仅保留历史审计，不再作为当前状态来源。
